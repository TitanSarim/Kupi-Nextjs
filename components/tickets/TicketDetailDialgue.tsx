...
const TicketDetailDialgue : React.FC<DialogProps> = ({ open, onClose, TicketData }) => {
    ...
    const busCompanyAmount = TicketData?.tickets && (TicketData?.tickets.priceDetails.kupiMarkup -  TicketData?.tickets.priceDetails.kupiProfit) - (TicketData?.tickets.priceDetails.carmaProfit - TicketData?.tickets.priceDetails.totalPrice )
    const kupiCommissionPercent = TicketData?.tickets && TicketData?.tickets.priceDetails.kupiCommissionPercentage + TicketData?.tickets.priceDetails.kupiMarkupPercentage
    const KupiAmount = TicketData?.tickets && TicketData?.tickets.priceDetails.kupiMarkup +  TicketData?.tickets.priceDetails.kupiProfit
                    ...
                    <div className='bg-white rounded-lg px-8 py-4 flex flex-col items-start justify-center gap-4 border-2'>
                        ...
                        <div className='w-full flex flex-wrap justify-between gap-4'>
                            ...
                            <div className='w-5/12'>
                                ...
                                <span>${TicketData?.tickets.priceDetails.carmaProfit.toFixed(1)}</span>
                            </div>
                            <div className='w-5/12'>
                                <p className='text-gray-600 font-light'>Kupi Commission</p>
                                {TicketData && TicketData?.tickets.priceDetails.kupiMarkupPercentage > 0 ? (
                                    <span>({TicketData?.tickets.priceDetails.kupiCommissionPercentage}% + {TicketData?.tickets.priceDetails.kupiMarkupPercentage}%) = {kupiCommissionPercent}%</span>
                                ) : (
                            ...
                            <div className='w-5/12'>
                                ...
                                {TicketData && TicketData?.tickets.priceDetails.kupiMarkup > 0 ? (
                                    <span>(${TicketData?.tickets.priceDetails.kupiMarkup.toFixed(1)} + ${TicketData?.tickets.priceDetails.kupiProfit.toFixed(1)}) = ${KupiAmount && KupiAmount.toFixed(1)}</span>
                                ) : (
                                        <span>${TicketData?.tickets.priceDetails.kupiProfit.toFixed(1)}</span>
                                )}
                        ...
                        <div className='flex w-full flex-row justify-between'>
                            ...
                            <p className='text-black font-semibold text-md'>${TicketData?.tickets.priceDetails.totalPrice.toFixed(1)} </p>
                        </div>
                    </div>
                </div>
                ...
                <div className='relative h-full passenger_info_dialgue bg-white rounded-lg px-8 py-4 flex flex-col items-start justify-start gap-4 border-2'>
                    ...
                    <div className='w-full flex flex-col gap-3'>
                        {TicketData?.passengerDetails?.map((passenger, i) => (
                            <div className='flex w-full flex-col justify-between border-b-2 pb-2' key={i}>
                                <div className='w-5/12'>
                                    <p className='text-gray-600 font-light'>Name</p>
