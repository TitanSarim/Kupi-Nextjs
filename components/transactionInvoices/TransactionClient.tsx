        ...
        <div className='flex flex-row items-center justify-center gap-4'>
            <div className='flex flex-row gap-10 border-2 border-gray-400 px-4 py-3 rounded-lg box-bg'>
                ...
                <label className='switch'>
                    <input type='checkbox' checked={selectedView} onChange={() => setSelectedView(prevChecked => !prevChecked)}/>
                    <span className='slider round'></span>
                </label>
            </div>
            {selectedView && <button onClick={() => setShowAddDialog(true)} className='bg-kupi-yellow py-3 px-4 outtline-none border-none rounded-lg'>Add Transaction</button>}
        </div>
        
        <AddTreansactionDialogue open={showAddDialog} onClose={handleAddTransaction}/>
