...
import { updateRoute, getRouteById } from "@/actions/route.action";
      ...
      try {
        if (routeId) {
          ...
          if (fetchedRoute) {
            ...
            setStops(
              fetchedRoute.stops.map((stop) => ({
                location: stop.location || {}, // Ensure location is set
                arrivalTime: stop.arrivalTime || "",
                departureTime: stop.departureTime || "",
              }))
            );
            setPrices(
              Array.isArray(fetchedRoute.pricing?.pricingData)
                ? fetchedRoute.pricing.pricingData.map((price: any) => ({
                    fromIndex: price.fromIndex,
                    toIndex: price.toIndex,
                    price: price.price ?? 0,
                  }))
                : []
            );
